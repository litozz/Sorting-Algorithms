...
#																																			#
#	Esto viene a cuenta del tercer parámetro de quickSort() ya que el primero siempre será 0 (si queremos ordenar todo el vector)			#
#############################################################################################################################################
		...
		v_der=quickSort(v[((izquierda+derecha)/2)+1:len(v)],0,len(v)-((izquierda+derecha)/2)-3,prof+1)#Ordeno la parte derecha de forma independiente
...
if (__name__=="__main__"):
	...
	#array=pivoteoLineal(m,0,len(m)-1)
	#print array
	m=[24,38,56,12,4,36,98,11,9,13,82,44]
	m=quickSort(m,0,len(m)-1,0)
	print(m)
	#print("hola")